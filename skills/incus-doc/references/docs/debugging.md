...
For information on debugging instance issues, see [How to troubleshoot failing instances](../howto/instances_troubleshoot/#instances-troubleshoot).
...
See the [RESTful API](../rest-api/) for available API.

## REST API through HTTPS

[HTTPS connection to Incus](../security/#security) requires valid
client certificate that is generated on first [[[`incus`]` `[`remote`]` `[`add`]]](../reference/manpages/incus/remote/add/#incus-remote-add-md). This
...
The files of the global [database](../database/#database) are stored under the [`./database/global`]
...
If you want to flush the content of the cluster database to disk, use the [`incus`]` `[`admin`]` `[`sql`]` `[`global`]` `[`.sync`] command, that will write a plain SQLite database file into
