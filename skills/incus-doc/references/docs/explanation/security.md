...
Incus is a daemon that can be accessed locally over a Unix socket or, if configured, remotely over a TLS socket.
...
By setting the [`core.https_address`] configuration option, you can expose the same API over the network on a TLS socket.
See [How to expose Incus to the network](../../howto/server_expose/#server-expose) for instructions.
...
See [Remote API authentication](../../authentication/#authentication) for details.
...
However, Incus offers several bridged NIC security features that can be used to control the type of traffic that an instance is allowed to send onto the network.
