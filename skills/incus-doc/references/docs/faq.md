...
See [How to configure your firewall](../howto/network_bridge_firewalld/#network-bridge-firewall) for more information about the problem and how to fix it.

Another frequent reason for connectivity issues is running Incus and Docker on the same host.
See [Prevent connectivity issues with Incus and Docker](../howto/network_bridge_firewalld/#network-incus-docker) for instructions on how to fix such issues.
...
You can enable it for remote access by following the instructions in [How to expose Incus to the network](../howto/server_expose/#server-expose).
...
See [Authenticate with the Incus server](../howto/server_expose/#server-authenticate) for instructions on how to authenticate using a trust token.
...
Yes, you can do this by using a [disk device](../reference/devices_disk/#devices-disk):
...
-   Pass [`shift=true`] to the [[[`incus`]` `[`config`]` `[`device`]` `[`add`]]](../reference/manpages/incus/config/device/add/#incus-config-device-add-md) call. This depends on the kernel and file system supporting either idmapped mounts (see [[[`incus`]` `[`info`]]](../reference/manpages/incus/info/#incus-info-md)).

-   Add a [`raw.idmap`] entry (see [Idmaps for user namespace](../userns-idmap/)).
...
To run Docker inside an Incus container, set the [`security.nesting`](../reference/instance_options/#instance-security:security.nesting) property of the container to [`true`]:
...
The [[[`incus`]]](../reference/manpages/incus/#incus-md) command stores its configuration under [`~/.config/incus`].
...
To see detailed information about what Incus is doing and what processes it is running, use the [[[`incus`]` `[`monitor`]]](../reference/manpages/incus/monitor/#incus-monitor-md) command.

For example, to show a human-readable output of all types of messages, enter the following command:

    incus monitor --pretty

See [[[`incus`]` `[`monitor`]` `[`--help`]]](../reference/manpages/incus/monitor/#incus-monitor-md) for all options, and [How to debug Incus](../debugging/) for more information.

## Why does Incus stall when creating an instance?

Check if your storage pool is out of space (by running [[[`incus`]` `[`storage`]` `[`info`]` `[`<pool_name>`]]](../reference/manpages/incus/storage/info/#incus-storage-info-md)).
In that case, Incus cannot finish unpacking the image, and the instance that you're trying to create shows up as stopped.

To get more insight into what is happening, run [[[`incus`]` `[`monitor`]]](../reference/manpages/incus/monitor/#incus-monitor-md) (see [How can I monitor what Incus is doing?](#faq-monitor)), and check [`sudo`]` `[`dmesg`] for any I/O errors.
...
When setting the [`bridge.mtu`] option on an Incus managed bridge network, Incus will create a dummy network interface named [`BRIDGE-mtu`].
