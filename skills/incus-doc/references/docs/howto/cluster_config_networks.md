...
The only configuration keys that may differ between networks on different members are [[[`bridge.external_interfaces`]]](../../reference/network_bridge/#network-bridge-options), [[[`parent`]]](../../reference/network_external/#network-external), [[[`bgp.ipv4.nexthop`]]](../../reference/network_bridge/#network-bridge-options) and [[[`bgp.ipv6.nexthop`]]](../../reference/network_bridge/#network-bridge-options).
...
1.  Define and configure the new network across all cluster members.
    ...
    If you run [[[`incus`]` `[`network`]` `[`list`]]](../../reference/manpages/incus/network/list/#incus-network-list-md), you can see that the network is marked as "pending".
...
To do so, you must specify different addresses for [`cluster.https_address`](../../server_config/#server-cluster:cluster.https_address) (the address for internal cluster traffic) and [`core.https_address`](../../server_config/#server-core:core.https_address) (the address for the REST API):
