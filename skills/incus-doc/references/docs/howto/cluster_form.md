...
See [Cluster members](../../explanation/clustering/#clustering-members) for more information.
...
2.  Authenticate with the cluster.
    ...
    ::: {#panel-0-QXV0aGVudGljYXRpb24gdG9rZW5z .sphinx-tabs-panel .group-tab aria-labelledby="tab-0-QXV0aGVudGljYXRpb24gdG9rZW5z" role="tabpanel" tabindex="0"}
    If you configured your cluster to use [authentication tokens](../../authentication/#authentication-token), you must generate a join token for each new member.
    ...
    This command returns a single-use join token that is valid for a configurable time (see [`cluster.join_token_expiry`](../../server_config/#server-cluster:cluster.join_token_expiry)).
...
4.  Configure server-specific settings (see [Member configuration](../../explanation/clustering/#clustering-member-config) for more information).

    You can accept the default values or specify custom values for each server.

Expand to see full examples for `incus admin init` on additional servers
