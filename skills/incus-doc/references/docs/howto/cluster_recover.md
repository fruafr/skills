...
See [Offline members and fault tolerance](../../explanation/clustering/#clustering-offline-members) and [Cluster healing](../cluster_manage/#cluster-automatic-evacuation) for more information.
...
Every Incus cluster has a specific number of members (configured through [`cluster.max_voters`](../../server_config/#server-cluster:cluster.max_voters)) that serve as voting members of the distributed database.
...
2.  Pick one of the listed database members that is still online as the new leader.
    Log on to the machine (if it differs from the one you are already logged on to).

3.  Make sure that the Incus daemon is not running on the machine.
...
4.  Log on to all other cluster members that are still online and stop the Incus daemon.
...
6.  Start the Incus daemon again on all machines, starting with the new leader.
...
See [Delete cluster members](../cluster_manage/#cluster-manage-delete-members).
...
You can edit the [Member roles](../../explanation/clustering/#clustering-member-roles) of the different members, but with the following limitations:
...
For example, if you have a cluster member that was removed uncleanly, it might not show up in [[[`incus`]` `[`cluster`]` `[`list`]]](../../reference/manpages/incus/cluster/list/#incus-cluster-list-md) but still be part of the Raft configuration.
