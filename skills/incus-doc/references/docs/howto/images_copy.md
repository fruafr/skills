...
See [[[`incus`]` `[`image`]` `[`copy`]` `[`--help`]]](../../reference/manpages/incus/image/copy/#incus-image-copy-md) for a list of all available flags.
...
If you have image files that use the required [Image format](../../reference/image_format/#image-format), you can import them into your image store.

There are several ways of obtaining such image files:

-   Exporting an existing image (see [Export an image to a file](../images_manage/#images-manage-export))

-   Building your own image using [`distrobuilder`] (see [Build an image](../images_create/#images-create-build))

-   Downloading image files from a [remote image server](../../reference/image_servers/#image-servers) (note that it is usually easier to [use the remote image](../images_remote/#images-remote) directly instead of downloading it to a file and importing it)

### Import from the local file system

To import an image from the local file system, use the [[[`incus`]` `[`image`]` `[`import`]]](../../reference/manpages/incus/image/import/#incus-image-import-md) command.
This command supports both [unified images](../../reference/image_format/#image-format-unified) (compressed file or directory) and [split images](../../reference/image_format/#image-format-split) (two files).
...
See [[[`incus`]` `[`image`]` `[`import`]` `[`--help`]]](../../reference/manpages/incus/image/import/#incus-image-import-md) for all available flags.
...
The image files must be provided as unified images (see [Unified tarball](../../reference/image_format/#image-format-unified)).
