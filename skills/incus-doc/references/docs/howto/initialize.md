...
Clustering (see [About clustering](../../explanation/clustering/#exp-clustering) and [How to form a cluster](../cluster_form/#cluster-form))

:   A cluster combines several Incus servers.
    The cluster members share the same distributed database and can be managed uniformly using the Incus client ([[[`incus`]]](../../reference/manpages/incus/#incus-md)) or the REST API.
...
Networking (see [About networking](../../explanation/networks/#networks) and [Network devices](../../reference/devices_nic/#devices-nic))
...
Storage pools (see [About storage pools, volumes and buckets](../../explanation/storage/#exp-storage) and [Storage drivers](../../reference/storage_drivers/#storage-drivers))
...
Remote access (see [Access to the remote API](../../explanation/security/#security-remote-access) and [Remote API authentication](../../authentication/#authentication))
...
Automatic image update (see [About images](../../image-handling/#about-images))
...
YAML [`incus`]` `[`admin`]` `[`init`] preseed (see [Non-interactive configuration](#initialize-preseed))
...
Especially the [[[`dir`] storage driver]](../../reference/storage_dir/#storage-dir), which is used by default, is slower than other drivers and doesn't provide fast snapshots, fast copy/launch, quotas and optimized backups.
...
Failure modes when overwriting entities are the same as for the [`PUT`] requests in the [REST API](../../rest-api/).
...
The supported keys and values of the various entities are the same as the ones documented in the [REST API](../../rest-api/), but converted to YAML for convenience.
