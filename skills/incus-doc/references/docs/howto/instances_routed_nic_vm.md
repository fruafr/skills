...
When adding a [routed NIC device](../../reference/devices_nic/#nic-routed) to an instance, you must configure the instance to use the link-local gateway IPs as default routes.
...
This [`netplan`] configuration adds the [static link-local next-hop addresses](../../reference/devices_nic/#nic-routed) ([`169.254.0.1`] and [`fe80::1`]) that are required.
...
Before you start your instance, make sure that you have [configured the parent network](../../reference/devices_nic/#nic-routed) to enable proxy ARP/NDP.
