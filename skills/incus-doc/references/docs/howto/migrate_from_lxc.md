...
Alternatively, you can use the `incus-migrate` tool within a LXC container to migrate it to Incus (see [How to import physical or virtual machines to Incus instances](../import_machines_to_instances/#import-machines-to-instances)). However, this tool does not migrate any of the LXC container configuration.

## Get the tool

If the tool isn't provided alongside your Incus installation, you can build it yourself. Make sure that you have `go` ([Go](../../requirements/#requirements-go)) installed and get the tool with the following command:
