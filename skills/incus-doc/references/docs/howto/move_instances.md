...
To move an instance from one Incus server to another, use the [`incus move`](../../reference/manpages/incus/move/#incus-move-md) command:

    incus move [<source_remote>:]<source_instance_name> <target_remote>:[<target_instance_name>]

Note

When moving a container, you must stop it first. See [Live migration for containers](#live-migration-containers) for more information.

When moving a virtual machine, you must either enable [Live migration for virtual machines](#live-migration-vms) or stop it first.

Alternatively, you can use the [`incus copy`](../../reference/manpages/incus/copy/#incus-copy-md) command if you want to duplicate the instance:
