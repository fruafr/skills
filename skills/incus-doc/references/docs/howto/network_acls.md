...
See [ACL groups](#network-acls-groups) for more information.
...
As a next step, [add rules](#network-acls-rules) to the ACL.
...
You cannot edit a rule (except if you [edit the full ACL](#network-acls-edit)), but you can delete rules with the following command:
...
-   Automatic default action for any unmatched traffic (defaults to [`reject`], see [Configure default actions](#network-acls-defaults)).
...
This feature is supported only for the [bridge network using](../network_bridge_firewalld/#network-bridge-firewall) [`nftables`] and the [OVN network](../../reference/network_ovn/#network-ovn).
...
If your network supports [network peers](../network_ovn_peers/), you can reference traffic to or from the peer connection by using a network subject selector in the format [`@<network_name>/<peer_name>`].
For example:
