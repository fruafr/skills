...
It requires a single external listen address (see [Requirements for listen addresses](#network-forwards-listen-addresses) for more information about which addresses can be forwarded, depending on the network that you are using).
...
Network forwards have the following properties:
  ...
  [`ports`]            port list    no         List of [port specifications](#network-forwards-port-specifications)
...
[[*!*](#network_forward-common:target_address)]
