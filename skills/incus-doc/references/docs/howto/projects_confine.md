...
See [Confined projects in a multi-user environment](../../explanation/projects/#projects-confined) for more information.
...
See [TLS client certificates](../../authentication/#authentication-tls-certs) for detailed information.
...
The client can then add the server as a remote in the usual way ([[[`incus`]` `[`remote`]` `[`add`]` `[`<server_name>`]` `[`<token>`]]](../../reference/manpages/incus/remote/add/#incus-remote-add-md) or [[[`incus`]` `[`remote`]` `[`add`]` `[`<server_name>`]` `[`<server_address>`]]](../../reference/manpages/incus/remote/add/#incus-remote-add-md)) and can only access the project or projects that have been specified.
...
If Incus has not been [initialized](../initialize/#initialize) at this point, it is automatically initialized (with the default settings).
