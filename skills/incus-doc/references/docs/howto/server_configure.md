...
See [Server configuration](../../server_config/#server) for all configuration options that are available for the Incus server.
...
In the [Server configuration](../../server_config/#server) option tables, options that apply to the cluster are marked with a [`global`] scope, while options that apply to the local server are marked with a [`local`] scope.
