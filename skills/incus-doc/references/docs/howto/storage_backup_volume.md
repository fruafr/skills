...
-   [Copy custom storage volumes](../storage_move_volume/#storage-copy-volume)
...
Therefore, the content of a custom storage volume is not stored when you [back up your instance](../instances_backup/#instances-backup).
...
To do so, set the [`snapshots.schedule`] configuration option for the storage volume (see [Configure storage volume settings](../storage_volumes/#storage-configure-volume)).
...
See the [Storage drivers](../../reference/storage_drivers/#storage-drivers) documentation for more information about those configuration options.
