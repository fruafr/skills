...
1.  Complete the following steps on the three machines to install the required LINSTOR components:
    ...
    3.  Enable the LINSTOR satellite service to ensure it is always started with the machine:

            sudo systemctl enable --now linstor-satellite

2.  Complete the following steps on the first machine (`server01`) to setup the LINSTOR controller and bootstrap the LINSTOR cluster:
    ...
    2.  Enable the LINSTOR controller service to ensure it is always started with the machine:

            sudo systemctl enable --now linstor-controller

    3.  Add the nodes to the LINSTOR cluster (replace `<server_1>`, `<server_2>` and `<server_3>` with the IP addresses of the respective machines). In this case `server01` is a combined node (controller + satellite), while the other two nodes are just satellites:
    ...
    4.  Verify that all nodes are online and that their node names match the node names in the Incus cluster:
    ...
    5.  Verify that all nodes have the desired features available. In this case we're interested in `LVMThin` and `DRBD`, which are available:
    ...
    7.  Verify that all storage pools are created and report the expected size:
    ...
    10.  Verify the LINSTOR resource group created by Incus:
    ...
    11.  To test the storage, create some volumes and instances:
    ...
    12.  Verify the LINSTOR view of the resources created by Incus:

            ~$ linstor resource-definition list --show-props Aux/Incus/name Aux/Incus/type Aux/Incus/content-type
            ╭─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────╮
