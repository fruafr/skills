...
By default, custom storage volumes use the [`filesystem`] [content type](../../explanation/storage/#storage-content-types).
...
-   Custom storage volumes of [content type](../../explanation/storage/#storage-content-types) [`block`] or [`iso`] cannot be attached to containers, but only to virtual machines.

-   To avoid data corruption, storage volumes of [content type](../../explanation/storage/#storage-content-types) [`block`] should never be attached to more than one virtual machine at a time.
...
Custom storage volumes with the content type [`block`] do not take a location (and their [device](../../reference/devices/#devices) name is optional, defaulting to the volume name):

    incus storage volume attach <pool_name> <block_volume_name> <instance_name> <device_name>

#### Attach the volume as a device

The [[[`incus`]` `[`storage`]` `[`volume`]` `[`attach`]]](../../reference/manpages/incus/storage/volume/attach/#incus-storage-volume-attach-md) command is a shortcut for adding a disk device to an instance.
...
See [disk device](../../reference/devices_disk/#devices-disk) for all available device options.

[]{#storage-configure-io}

#### Configure I/O limits

When you attach a storage volume to an instance as a [disk device](../../reference/devices_disk/#devices-disk), you can configure I/O limits for it.
To do so, set the [`limits.read`], [`limits.write`] or [`limits.max`] properties to the corresponding limits.
See the [Type: disk](../../reference/devices_disk/#devices-disk) reference for more information.
...
To do so, you must set the corresponding [server configuration](../../server_config/#server-options-misc):
...
The default [storage volume type](../../explanation/storage/#storage-volume-types) is [`custom`], so you can leave out the [`<volume_type>/`] when configuring a custom storage volume.
...
In general, the defaults set on a storage pool level (before the volume was created) can be overridden through the volume configuration, and the volume configuration can be overridden through the instance configuration (for storage volumes of [type](../../explanation/storage/#storage-volume-types) [`container`] or [`virtual-machine`]).
...
The resulting table contains the [storage volume type](../../explanation/storage/#storage-volume-types) and the [content type](../../explanation/storage/#storage-content-types) for each storage volume in the pool.
...
In both commands, the default [storage volume type](../../explanation/storage/#storage-volume-types) is [`custom`], so you can leave out the [`<volume_type>/`] when displaying information about a custom storage volume.
