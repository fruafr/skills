...
The easiest way to install Incus is to [install one of the available packages](#installing-from-package), but you can also [install Incus from the sources](#installing-from-source).

After installing Incus, make sure you have an [`incus-admin`] group on your system.
Users in this group can interact with Incus.
See [Manage access to Incus](#installing-manage-access) for instructions.
...
The client tool ([[[`incus`]]](../reference/manpages/incus/#incus-md)) is available on most platforms.
...
For more information: [Idmaps for user namespace](../userns-idmap/#userns-idmap)
...
The default setup should work fine for most users, but if you intend to run many containers on your system you may wish to apply some custom [`sysctl`] settings [as suggested in the production deployments guide](../reference/server_settings/).
...
available for Incus development. Additionally, Incus requires a modern Golang (see [Go](../requirements/#requirements-go))
version to work.
...
****NOTE:**** The version of [`golang-go`] in your version of Debian or Ubuntu may not be sufficient to build Incus (see [Go](../requirements/#requirements-go)).
...
See [Access to the Incus daemon](../explanation/security/#security-daemon-access) for more information.
