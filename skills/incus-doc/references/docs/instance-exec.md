...
To run commands inside your instance, use the [[[`incus`]` `[`exec`]]](../reference/manpages/incus/exec/#incus-exec-md) command.
...
To run a single command from the terminal of the host machine, use the [[[`incus`]` `[`exec`]]](../reference/manpages/incus/exec/#incus-exec-md) command:
...
:   To set the [`ENVVAR`] environment variable to [`VALUE`] in the instance, set the [`environment.ENVVAR`] instance option (see [`environment.*`](../reference/instance_options/#instance-miscellaneous:environment.*)):
