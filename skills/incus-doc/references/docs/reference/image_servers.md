...
The [[[`incus`]]](../manpages/incus/#incus-md) CLI command comes pre-configured with the following default remote image server:
...
:   Incus servers that are used solely to serve images and do not run instances themselves.
    ...
    To make an Incus server publicly available over the network on port 8443, set the [`core.https_address`](../../server_config/#server-core:core.https_address) configuration option to [`:8443`] and do not configure any authentication methods (see [How to expose Incus to the network](../../howto/server_expose/#server-expose) for more information).
...
:   Regular Incus servers that you can manage over a network, and that can also be used as image servers.
    ...
    See [How to expose Incus to the network](../../howto/server_expose/#server-expose) and [Remote API authentication](../../authentication/#authentication) for more information.
