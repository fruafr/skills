...
See [Remote API authentication](../authentication/#authentication) for information about how to access the API remotely.

Tip

-   For examples on how the API is used, run any command of the Incus client ([[[`incus`]]](../reference/manpages/incus/#incus-md)) with the [`--debug`] flag.
    The debug information displays the API calls and the return values.

-   For quickly querying the API, the Incus client provides a [[[`incus`]` `[`query`]]](../reference/manpages/incus/query/#incus-query-md) command.
...
See [REST API](../api/) for a convenient web rendering of it.
